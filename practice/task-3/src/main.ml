...
let axiom_num stmt = match stmt with Binop(Impl, a1, Binop(Impl, b, a2)) when a1 = a2 -> 1
                   | Binop(Impl, Binop(Impl, a1, b1), Binop(Impl, Binop(Impl, a2, Binop(Impl, b2, c1)),Binop(Impl, a3, c2))) 
                      when a1 = a2 && a2 = a3 && b1 = b2 && c1 = c2 -> 2 | Binop(Impl, a1, Binop(Impl, b1, Binop(Conj, a2, b2))) when a1 = a2 && b1 = b2 -> 3
...
let update_assumption result stmt alpha = 
	result := stmt::!result;	
	result := Binop(Impl, stmt, Binop(Impl, alpha, stmt))::!result;
	result := Binop(Impl, alpha, stmt)::!result;
  (!result);;

let update_axiom result stmt alpha = 
	result := stmt::!result;
	result := Binop(Impl, stmt, Binop(Impl, alpha, stmt))::!result;
	result := Binop(Impl, alpha, stmt)::!result;
  (!result);;
...
let update_modus_ponens result hshtbl_ind stmt id1 id2 alpha = 
  let stmt1 = (Hashtbl.find hshtbl_ind id2) in
  		result := Binop(Impl, Binop(Impl, alpha, stmt1), Binop(Impl, Binop(Impl, alpha,	Binop(Impl, stmt1, stmt)), Binop(Impl, alpha, stmt)))::!result;
		result := Binop(Impl, Binop(Impl, alpha, Binop(Impl, stmt1, stmt)), Binop(Impl, alpha, stmt))::!result;
		result := Binop(Impl, alpha, stmt)::!result;
  (!result);;

let update_alpha result alpha =
	result := Binop(Impl, 
					Binop(Impl, alpha, Binop(Impl, alpha, alpha)),  
					Binop(Impl, Binop(Impl, alpha, Binop(Impl, Binop(Impl, alpha, alpha), alpha)), Binop(Impl, alpha, alpha)))::!result;    
	result := Binop(Impl, alpha, Binop(Impl, alpha, alpha))::!result;	
	result := Binop(Impl, alpha, Binop(Impl, Binop(Impl, alpha, alpha), alpha))::!result;
	result := Binop(Impl, Binop(Impl, alpha, Binop(Impl, Binop(Impl, alpha, alpha), alpha)), Binop(Impl, alpha, alpha))::!result;
	result := Binop(Impl, alpha, alpha)::!result;	
(!result);;
...
let rec string_of_tree_canon stmt = 
    let s_op op = match op with Conj -> "&" | Disj -> "|" | Impl -> "->" in
        let rec s_t t = match t with
            | Var v -> v
            | Neg t -> "(!"^(s_t t)^")"
            | Binop (op,l,r) -> "("^(s_t l)^(s_op op)^(s_t r)^")" in s_t stmt;;
    ...
    if (stmt <> element) then begin 
      let current_reason = get_reason stmt hshtbl_assm hshtbl_mp hshtbl_stmt in 
        (match current_reason with 
         | Assumption _ -> result := (update_assumption result stmt element)   
         | Axiom _ -> result := (update_axiom result stmt element)
         | ModusPonens (x, y) -> result := (update_modus_ponens result hshtbl_ind stmt x y element)
        | NoProof -> ())  
    end else begin 
      result := (update_alpha result element);
    end;
    Hashtbl.add hshtbl_ind !id stmt;
    add_statement stmt;
    incr(id);
  end in 
...
let brute_force_check_tautology var_hshtbl rev_varhshtbl tree = 
  let result = ref true in 
    let variable_value = Hashtbl.create 255 in 
      let mask = ref 0 in 
        while ((!mask < (1 lsl !variable_size)) && !result) do 
...
let head = input_line ic;;
let header_string = build_general_formula_of_string (head);; (* transfom a,b|-c in a->b->c *)
...
let substitute_in_scheme_one_list scheme_list stmt = 
	(* print_endline "here2\n"; *)
  ...
  | Binop (op, l, r) -> let (value_left, proof_left) = generate_atom_proof base_proofs_map l mask in 
                          let (value_right, proof_right) = generate_atom_proof base_proofs_map r mask in
                            concatanate_proofs 
                              base_proofs_map 
                              op 
                              value_left proof_left value_right proof_right
                              l r
                                   ...
                                   let new_proof1 = apply_deduction assm_list1 element1 h1 in 
                                     let new_proof2 = apply_deduction assm_list2 element2  h2 in 
                                      let base_proof_scheme = StringMap.find "exclude" base_proofs_map in
											let proof_with_exclusion = List.append new_proof1 new_proof2 in 
												let proof_with_exclusion = List.append proof_with_exclusion (substitute_in_scheme_list base_proof_scheme variable stmt_to_proof)
                                          in
...
if (var_id = 0) then begin 
    atom_proof_list
  end else if(var_id = 1) then begin 
	reduce_assumption base_proofs_map atom_proof_list 0 0 stmt_to_proof 
  end else  begin
...
if (not res) then begin
  ...
  fprintf oc "%s\n" head;
  let proof_map = load_base_proofs StringMap.empty base_proof_names in 
    let almost_perfect_proof = List.hd (generate_proof proof_map header_tree) in 
      let perfect_proof = make_perfect_proof almost_perfect_proof !assumption_count in 
        List.iter (fun value -> 
          fprintf oc "%s\n" (string_of_tree_canon value) 
        ) perfect_proof;
end;;
